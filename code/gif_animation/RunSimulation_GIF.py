...
CONTROL_MODE = "DOWN_EQ_ALINEA" # NO_CONTROL, DOWN_ALINEA, UP_ALINEA, DOWN_EQ_ALINEA, UP_EQ_ALINEA
...
RAMP_SENSOR_MATCHING = {k: [f"e2_E{k[2:]}_{i}" for i in range(8)] for k in ["JE2", "JE3", "JE4", "JE4b", "JE5"] if k.startswith("JE")}
SUMO_CONFIG_FILE = "../model/Configuration.sumocfg"
    # CONTROL RELEVANT
K_R = 70/60  # veh/min
...
VEHICLE_AV_LENGTH = SPAWN_MOTORCYCLE_PERCENTAGE*AV_MOTORCYCLE_LENGTH+SPAWN_CAR_PERCENTAGE*AV_CAR_LENGTH
...
def execute_ramp_metering_control(ramp, control_mode, current_time):
    if current_time > RECORDING_CONTROL_STATS_START_TIME:
        if current_time % SIGNAL_CYCLE_DURATION == 0:  # every SIGNAL_CYCLE_DURATION seconds
            ...
            if control_mode == "NO_CONTROL":
                ...
                q_rate[ramp], metering_rate[ramp] = control_EQ_ALINEA(
                    ramp=ramp,
                    q_previous_rate=traffic_data[ramp]["real_Q_flow"][-2],  # Pass the whole dictionary
                    q_4_previous_rates = traffic_data[ramp]["real_Q_flow"][-2] + traffic_data[ramp]["real_Q_flow"][-62] +traffic_data[ramp]["real_Q_flow"][-122] + traffic_data[ramp]["real_Q_flow"][-182],
                    occupancy_measured=traffic_data[ramp]["Upstream_O_Measured"][-1],
                ...
                q_rate[ramp], metering_rate[ramp] = control_EQ_ALINEA(
                    ...
                    q_previous_rate=traffic_data[ramp]["real_Q_flow"][-2],  # Pass the whole dictionary
                    q_4_previous_rates = traffic_data[ramp]["real_Q_flow"][-2] + traffic_data[ramp]["real_Q_flow"][-62] +traffic_data[ramp]["real_Q_flow"][-122] + traffic_data[ramp]["real_Q_flow"][-182],
                    occupancy_measured=traffic_data[ramp]["Downstream_O_Measured"][-1],# downstream detector are used here
                    previous_metering_rates=traffic_data[ramp]["metering_rates"],
                    queue_length=traffic_data[ramp]["queue_lengths"][-1],
                    detector_length=traffic_data[ramp]["detector_length"])     
...
def control_EQ_ALINEA(ramp, q_previous_rate, q_4_previous_rates, occupancy_measured, previous_metering_rates, queue_length, detector_length):
    """
    ALINEA control logic for a single ramp.
    ...
    # BOUNDARY CONDITION I: Sufficient time for on-ramp dequeuing
        ...
        metering_rate = (q_tau-q_4_previous_rates)*VEHICLE_AV_ACC_TIME/SIGNAL_CYCLE_DURATION
        metering_rate = min(1.0, math.floor(metering_rate * 10) / 10)
    # BOUNDARY CONDITION II: Maximum waiting times
    elif len(previous_metering_rates) >= 180 and all(value == 0 for value in previous_metering_rates[-180:]): #240==4cycle*60*1(step)
        metering_rate = R_B
...
def generateInitialVehicleData():
    vehicles_data = defaultdict(lambda: {
...
def record_vehicle_data(step, vehicles_data ):
    ...
    for vehicle in arrived_vehicles:
        ...
        if vehicles_data[vehicle]["EntryTime"] is not np.nan:
            vehicles_data[vehicle]["ExitTime"] = step/STEPS_PER_SECOND  # Record the exit time
            vehicles_data[vehicle]["TravelTime"] = (  # Calculate travel time as the difference between exit and entry times
...
def record_upstream_traffic_data(ramp):
    ...
    Upstream_weightO = (1/3 * Upstream_occupL + 1/3 * Upstream_occupM + 1/3 * Upstream_occupR)  # Normalize to [0-1]
    ...
    Upstream_Interval_O_Measured = ( 1/3 * Upstream_Interval_O_L + 1/3 * Upstream_Interval_O_M + 1/3 * Upstream_Interval_O_R )
    ...
    traffic_data[ramp]["Upstream_Interval_O_Measured"].append( Upstream_Interval_O_Measured )
    ...
    if len(traffic_data[ramp]["Upstream_occupancy_memory"]) > STATE_MEMORY_HORIZON:
        ...
        traffic_data[ramp]["Upstream_occupancy_memory"] = traffic_data[ramp]["Upstream_occupancy_memory"][1:]
    ...
    if (Upstream_vehNL + Upstream_vehNM + Upstream_vehNR) > 0:
        Upstream_weightS = (Upstream_speedL * Upstream_vehNL + Upstream_speedM * Upstream_vehNM + Upstream_speedR * Upstream_vehNR) / (Upstream_vehNL + Upstream_vehNM + Upstream_vehNR)
        ...
        if len(traffic_data[ramp]["Upstream_speed_memory"]) > STATE_MEMORY_HORIZON:
            # delete first element
            traffic_data[ramp]["Upstream_speed_memory"] = traffic_data[ramp]["Upstream_speed_memory"][1:]
    ...
    traffic_data[ramp]["Upstream_Interval_average_speeds"].append( (Upstream_Interval_Speed_L + Upstream_Interval_Speed_M + Upstream_Interval_Speed_R) / 3 )
    # Compute Entry flow
    Upstream_average_flow =  (Upstream_vehNL + Upstream_vehNM + Upstream_vehNR) / 3 * (3600)
    ...
    Upstream_Interval_average_flow =  (Upstream_Interval_vehNL + Upstream_Interval_vehNM + Upstream_Interval_vehNR) / (3 * 10) * (3600)   # 10 is the detector period
    traffic_data[ramp]["Upstream_Interval_average_flow"].append(Upstream_Interval_average_flow)
...
def record_downstream_traffic_data(ramp):
    ...
    Downstream_weightO = ( 1/3 * Downstream_occupL + 1/3 * Downstream_occupM + 1/3 * Downstream_occupR)  # Normalize to [0-1]
    # Test for Interval Occupancy
    ...
    Downstream_Interval_O_Measured = ( 1/3 * Downstream_Interval_O_L + 1/3 * Downstream_Interval_O_M + 1/3 * Downstream_Interval_O_R )
    ...
    traffic_data[ramp]["Downstream_Interval_O_Measured"].append( Downstream_Interval_O_Measured )
    ...
    if len(traffic_data[ramp]["Downstream_occupancy_memory"]) > STATE_MEMORY_HORIZON:
        # delete first element
        traffic_data[ramp]["Downstream_occupancy_memory"] = traffic_data[ramp]["Downstream_occupancy_memory"][1:]
    ...
    Downstream_Interval_vehNL = traci.lanearea.getLastIntervalVehicleNumber(detectors[4])
    Downstream_Interval_vehNM = traci.lanearea.getLastIntervalVehicleNumber(detectors[3])
    Downstream_Interval_vehNR = traci.lanearea.getLastIntervalVehicleNumber(detectors[2])
    if (Downstream_vehNL + Downstream_vehNM + Downstream_vehNR) > 0:
        Downstream_weightS = (Downstream_speedL * Downstream_vehNL + Downstream_speedM * Downstream_vehNM + Downstream_speedR * Downstream_vehNR) / (Downstream_vehNL + Downstream_vehNM + Downstream_vehNR)
        ...
        if len(traffic_data[ramp]["Downstream_speed_memory"]) > STATE_MEMORY_HORIZON:
            # delete first element
            traffic_data[ramp]["Downstream_speed_memory"] = traffic_data[ramp]["Downstream_speed_memory"][1:]
    ...
    traffic_data[ramp]["Downstream_Interval_average_speeds"].append( (Downstream_Interval_Speed_L + Downstream_Interval_Speed_M + Downstream_Interval_Speed_R) / 3 )
    # Downstream Flow
    Downstream_average_flow =  (Downstream_vehNL + Downstream_vehNM + Downstream_vehNR) / 3 * (3600)
    ...
    Downstream_Interval_average_flow =  (Downstream_Interval_vehNL + Downstream_Interval_vehNM + Downstream_Interval_vehNR) / (3 * 10) * (3600)   # 10 is the detector period
    traffic_data[ramp]["Downstream_Interval_average_flow"].append(Downstream_Interval_average_flow)
...
# LAUNCH SUMO
sumoCmd = [sumoBinary, "-c", SUMO_CONFIG_FILE, "--start", "--quit-on-end", "--time-to-teleport", "-1",]
traci.start(sumoCmd)
...
for ramp in RAMP_SENSOR_MATCHING:
    update_ramp_signal_control_logic(metering_rate[ramp], SIGNAL_CYCLE_DURATION, ramp)
    traffic_data[ramp]["detector_length"] = traci.lanearea.getLength( RAMP_SENSOR_MATCHING[ramp][0] )
...
for step in range(0, SIMULATION_DURATION*STEPS_PER_SECOND):
    ...
    # GIF
    traci.gui.setOffset("View #0", 18131, 17414)
    traci.gui.setZoom('View #0', 4500)
    if current_time>1000:
        traci.gui.screenshot('View #0', "figures/foto_"+"{0:0=3d}".format(int(current_time))+"Test.png")
